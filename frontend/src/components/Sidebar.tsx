...
const menuItems = [
  ...
  { icon: Briefcase, label: 'Referrals', path: '/jobs' },
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  const [user, setUser] = useState<{ name: string; branch: string }>({ name: '', branch: '' });
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        setUser({ name: data.name, branch: data.department });
      } catch (err) {
        ...
        <section className="p-6 flex items-center space-x-4">
          ...
          <div>
            {loading ? (
              ...
              <>
                <h3 className="font-semibold text-primary-500">{user.name || 'No Name'}</h3>
                <p className="text-sm text-primary-400">{user.branch || 'No Branch'}</p>
              </>
            )}
          </div>
        </section>
